...
### Citations
- Always use `[Source Title](URL)` for web sources
- Use `[Ddoc Title](blockchain-link)` for cross-referencing other ddocs
...
### Source Citations
- Link to original sources using standard markdown hyperlinks
- Note if information is outdated (check publication dates)
