...
export const submitEvent = async (event: Event): Promise<void> => {
  try {
    ...
    await EventsModel.markFailed(event._id, error instanceof Error ? error.message : String(error));
    throw error;
  }
};
...
export const resolveEvent = async (event: Event): Promise<{ resolved: boolean }> => {
  try {
    ...
    const fileEntity = await FilesModel.findByIdIncludingDeleted(fileId);
    if (!fileEntity) {
      logger.warn(`File ${fileId} not found, cannot resolve`);
      return { resolved: false };
    }
    const result = await resolveFileOp(fileEntity.portalAddress as `0x${string}`, userOpHash);
    if (!result) {
      return { resolved: false };
    }
    ...
    switch (type) {
      case "create": {
        ...
        await EventsModel.markResolved(event._id);
        logger.info(`File ${fileEntity.ddocId} create resolved successfully`);
        break;
      }
      case "update": {
        ...
        await EventsModel.markResolved(event._id);
        logger.info(`File ${fileEntity.ddocId} update resolved successfully`);
        break;
      }
      case "delete": {
        ...
        await FilesModel.update(
          fileId,
          { syncStatus: "synced", isDeleted: 1, onchainVersion: fileEntity.localVersion },
          fileEntity.portalAddress,
        );
        await EventsModel.markResolved(event._id);
