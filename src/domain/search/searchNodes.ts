...
export default async function searchNodes(params: SearchNodesParams): Promise<SearchNodesResult> {
  ...
  const totalResult = await QueryBuilder.selectOne<{ count: number }>(countSql, [`%${query}%`, portalAddress]);
